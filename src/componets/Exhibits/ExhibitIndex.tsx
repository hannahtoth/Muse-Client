...
fetchArt = async (e: React.FormEvent<HTMLFormElement>):Promise<any> => {
    e.preventDefault()
 let res =  await fetch(`https://api.artic.edu/api/v1/artworks/search?q=${this.state.searchTerm}&query[term][is_public_domain]=true`)
 let json = await res.json()
 let fetchedArt = await json.data.map((data:any) => {
     const {title, id, } = data
     const {alt_text, } = data.thumbnail
    //  const {image_id} = data.api_link
...
this.setState({
    ...
    render() {
        return(
            ...
            <form onSubmit={(e) => {this.fetchArt(e)}}>
                <input onChange={(e) =>{this.handleSearchInput(e)}} value={this.state.searchTerm} type="text" placeholder="search"></input>
